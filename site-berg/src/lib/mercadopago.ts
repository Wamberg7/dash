    ...
    const response = await fetch(
      `${apiUrl}/api/create-pix-payment`,
      {
        ...
        body: JSON.stringify({
          accessToken: accessToken.trim(),
          paymentData: {
            transaction_amount: payment.transaction_amount ? Number(payment.transaction_amount.toFixed(2)) : null,
            description: payment.description,
            payer: {
              email: payment.payer.email,
              first_name: payment.payer.first_name || payment.payer.email.split('@')[0],
              last_name: payment.payer.last_name || payment.payer.email.split('@')[0],
            },
          },
        }),
      },
